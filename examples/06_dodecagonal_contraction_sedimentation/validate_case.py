#!/usr/bin/env python3
"""Run static consistency checks for the generated dodecagon case."""
...
def main() -> int:
    ...
    dictionary_paths.extend(CASE_DIR / "0.org" / name for name in ("U", "p", "f", "lambda"))
    for path in dictionary_paths:
        check_balanced(path)

    for field_name in ("U", "p", "f", "lambda"):
    ...
    if not re.search(r"\bnSolidsInDomain\s+1000\s*;", without_comments(hfdib_text)):
        raise AssertionError("The intentional 1000-particle validation cap changed")
    ...
    legacy_names = {"xMinWall", "xMaxWall", "zMinWall", "zMaxWall"}
    if any(legacy_names.intersection(boundary_patch_names(CASE_DIR / "0.org" / name)) for name in ("U", "p", "f", "lambda")):
        raise AssertionError("A rectangular example-05 patch name remains in 0.org")

    print(
        "Case dictionaries consistent: 5 CFD patches, 37 DEM walls, "
        "660000 cells, a 12-vertex polygonPrism, and no example-05 wall "
        "patch names."
    )
    return 0
