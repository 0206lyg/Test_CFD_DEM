#!/usr/bin/env python3
"""Generate and validate the static dodecagonal contraction geometry.

The CFD mesh and all DEM finite-wall polygons are produced from the same
geometry constants.  This prevents the shoulder/side-wall coordinate drift
that is possible when blockMeshDict and HFDIBDEMDict are maintained by hand.
...
def build_mesh() -> MeshData:
    ...
    def add_block(
        ...
        if region == "downstream":
            patches["bottomWall"].append(lower_face)
            if i == small_lo:
            ...
            patches["topOpen"].append(upper_face)
            inside_opening = (
...
def validate_mesh(mesh: MeshData) -> None:
    ...
    expected_patch_areas = {
        "bottomWall": polygon_area(SMALL_APOTHEM),
        "topOpen": polygon_area(LARGE_APOTHEM),
...
def build_walls() -> List[Wall]:
    ...
    bottom_vertices = tuple(
        (x, Y_BOTTOM, z) for x, z in regular_polygon_vertices(SMALL_APOTHEM)
    )
    walls.append(
        Wall(
            "bottomWall",
...
def validate_walls(walls: Sequence[Wall]) -> None:
    ...
    for wall in walls:
        ...
        elif wall.kind == "bottom":
            bottom_area += polygon_area_3d(wall.vertices, wall.normal)
    ...
    expected_areas = {
        ...
        "bottom": N_SIDES * SMALL_APOTHEM**2 * math.tan(math.pi / N_SIDES),
    }
    actual_areas = {
...
def render_block_mesh_dict(mesh: MeshData) -> str:
    lines: List[str] = [
        "/*--------------------------------*- C++ -*----------------------------------*\\",
        "| Generated regular-dodecagon contraction mesh                              |",
        "\\*---------------------------------------------------------------------------*/",
        "FoamFile",
        "{",
        "    version     2.0;",
    ...
    for block in mesh.blocks:
        ...
        lines.append(f"    hex ({vertex_text}) ({cell_text}) simpleGrading (1 1 1)")
...
def render_collision_patches(walls: Sequence[Wall]) -> str:
    lines = [
        "// Generated by generate_dodecagon_geometry.py. Do not edit by hand.",
        "// 12 small sides + 12 large sides + 12 shoulders + 1 bottom = 37.",
        "",
    ]
...
def main() -> int:
    ...
    print(
        ...
        f"{len(walls)} DEM walls (8 side walls on the legacy fast path)."
    )
    return 0
